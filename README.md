...
Таблицы повторяют Kendo UI Grid из BpDazApp — тему Bootstrap сборки 2017.2.504. Значения
собраны в `src/components/kendoTable.ts`: серая шапка `#f5f5f5` обычным начертанием,
разделители колонок и рамка `#ccc`, чередование строк, подсветка `#ebebeb`, кегль 14 px.
Цвета и отступы не подобраны на глаз — старый грид отрисован в браузере, и значения сняты
с него через `getComputedStyle`. Плашки в ячейках (`KendoLabel`) — это `.label` из
Bootstrap 3, которым в BpDazApp красили значения внутри грида.

Тулбар и пагинация под таблицей остались в оформлении PassBureau: в BpDazApp пейджер был
частью виджета грида, у нас это отдельная панель со своей логикой.
